...
class TentacleSpike: SKSpriteNode {
    let fireballSpeed = 300
    
    class func spikeAtPosition(position: CGPoint) -> TentacleSpike{
        
        let spike = TentacleSpike(imageNamed: "Beach_Ball_4")
        spike.position = position
        spike.name = "spike"
        
        
        spike.setupAnimation()
        spike.setupPhysicsBody()
        spike.setScale(0.15)
        return spike
