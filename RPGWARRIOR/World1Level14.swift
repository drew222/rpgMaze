...
class World1Level14: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        theHero = HeroClass.makeHero(position: CGPointMake(self.frame.minX + 30, 30))
        ...
        //seashells
        //outter = rows
        for i in stride(from: 80, to: Int(self.frame.maxY - 140), by: 160) {
            for k in stride(from: 20, to: Int(self.frame.maxX) - 60, by: 30) {
                self.addChild(MineNode.mineAtPos(position: CGPointMake(CGFloat(k), CGFloat(i))))
            }
        }
        for i in stride(from: 160, to: Int(self.frame.maxY - 140), by: 160) {
            for k in stride(from: 60, to: Int(self.frame.maxX) - 0, by: 30) {
                self.addChild(MineNode.mineAtPos(position: CGPointMake(CGFloat(k), CGFloat(i))))
            }
        }
        //if isPlus{
        ...
        // }
        //******
        for k in stride(from: 20, to: Int(self.frame.maxX) - 60, by: 40) {
            attackSpots.append(CGPointMake(CGFloat(k), 40))
        }
        attackSpots.append(CGPointMake(0, 0))
        for k in stride(from: Int(self.frame.maxX) - 20, to: 60, by: -40) {
    ...
    override func update(_ currentTime: CFTimeInterval) {
        ...
        _ = self.childNode(withName: "hero")
        ...
        if currentTime - lastFireball  > wizardAttackSpeed && !inkSplatted && !droppedChest && !droppedItem{
            ...
            if whichSpot < attackSpots.count {
                if !(attackSpots[whichSpot].x == 0 && attackSpots[whichSpot].y == 0){
                    theWizard!.shootFireball(aPosition: attackSpots[whichSpot])
                }
                whichSpot += 1
            }
        }
        ...
        if (theWizard!.isDead || theHero!.life! <= 0) && !levelOver{
            ...
            if (self.childNode(withName: "gold") == nil && self.childNode(withName: "item") == nil && droppedItem) || theHero!.life! <= 0{
                ...
                if theHero!.life! <= 0 {
                    ...
                    if (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel < 14{
                        persistentData.set(14, forKey: "highestLevel")
                        (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel = 14
                    }
                    //&&
                ...
                if theWizard!.isDead && droppedChest && (self.childNode(withName: "chest") as! TreasureChest).open{
                    dropLoot(level: "level14", scene: self, position: CGPointMake(self.frame.midX, self.frame.midY), size: CGSizeMake(30, 30))
                    droppedItem = true
