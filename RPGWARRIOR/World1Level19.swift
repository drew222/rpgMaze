...
class World1Level19: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        //bot shells
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX, 65)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX - 45, 65)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX + 45, 65)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX + 45, 25)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX - 45, 25)))
        ...
        if (theKraken!.isDead || theHero!.life ?? <#default value#> <= 0) && !levelOver{
            ...
            if (self.childNode(withName: "gold") == nil && self.childNode(withName: "item") == nil && droppedItem) || theHero!.life ?? <#default value#> <= 0{
                ...
                if theHero!.life ?? <#default value#> <= 0 {
                    ...
                    if (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel < 19{
                        persistentData.set(19, forKey: "highestLevel")
                        (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel = 19
                    }
                    //&&
                ...
                if theKraken!.isDead && droppedChest && (self.childNode(withName: "chest") as! TreasureChest).open{
                    dropLoot(level: "level19", scene: self, position: CGPointMake(self.frame.midX, self.frame.midY), size: CGSizeMake(30, 30))
                    droppedItem = true
