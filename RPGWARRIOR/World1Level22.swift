...
class World1Level22: SKScene, SKPhysicsContactDelegate {
    
    var timeSinceCrabAdded : TimeInterval = 0
    var addCrabTimeInterval : TimeInterval = 0.25
        ...
        for touch in touches{
            if !inkSplatted{
                ...
                if self.childNode(withName: "yesText")!.contains((touch ).location(in: self)){
                    let newLevel1 = World1Level22(size: self.frame.size)
    ...
    override func update(_ currentTime: CFTimeInterval) {
        ...
        let yMatch = CGFloat(randomWithMin(min: Int(self.frame.minY), max: Int(self.frame.maxY)))
        ...
        if (self.timeSinceCrabAdded > self.addCrabTimeInterval && !self.levelOver && !self.droppedItem && !inkSplatted && !droppedChest) {
            self.addChild(MudCrab.crabDash(startPos: CGPointMake(self.frame.minX, yMatch), endPosition: CGPointMake(self.frame.maxX, yMatch)))
        ...
        if (theBomber!.isDead || theHero!.life ?? <#default value#> <= 0) && !levelOver{
            ...
            if (self.childNode(withName: "gold") == nil && self.childNode(withName: "item") == nil && droppedItem) || theHero!.life ?? <#default value#> <= 0{
                ...
                if theHero!.life ?? <#default value#> <= 0 {
                    ...
                    if (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel < 22{
                        persistentData.set(22, forKey: "highestLevel")
                        (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel = 22
                    }
                    //&&
                ...
                if theBomber!.isDead && droppedChest && (self.childNode(withName: "chest") as! TreasureChest).open{
                    dropLoot(level: "level22", scene: self, position: CGPointMake(self.frame.midX, self.frame.midY), size: CGSizeMake(30, 30))
                    droppedItem = true
