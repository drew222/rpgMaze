...
class World1Level22: SKScene, SKPhysicsContactDelegate {
    ...
    override func update(currentTime: CFTimeInterval) {
        ...
        let yMatch = CGFloat(randomWithMin(Int(self.frame.minY), max: Int(self.frame.maxY)))
        ...
        if (theBomber!.isDead || theHero!.life <= 0) && !levelOver{
            ...
            if (self.childNodeWithName("gold") == nil && self.childNodeWithName("item") == nil && droppedItem) || theHero!.life <= 0{
                ...
                if theHero!.life <= 0 {
                    ...
                    if (self.userData?.objectForKey("menu") as! MainMenuScene).highestLevel < 22{
                    persistentData.setObject(22, forKey: "highestLevel")
                        (self.userData?.objectForKey("menu") as! MainMenuScene).highestLevel = 22
                    }
                    //&&
                ...
                if theBomber!.isDead && droppedChest && (self.childNodeWithName("chest") as! TreasureChest).open{
                    dropLoot("level22", scene: self, position: CGPointMake(self.frame.midX, self.frame.midY), size: CGSizeMake(30, 30))
                    droppedItem = true
