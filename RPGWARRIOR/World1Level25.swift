        ...
        if (theKraken!.isDead || theHero!.life <= 0) && !levelOver{
            ...
            if (self.childNodeWithName("gold") == nil && self.childNodeWithName("item") == nil && droppedItem) || theHero!.life <= 0{
                ...
                if theHero!.life <= 0 {
                    ...
                    if (self.userData?.objectForKey("menu") as! MainMenuScene).highestLevel < 25{
                    persistentData.setObject(25, forKey: "highestLevel")
                        (self.userData?.objectForKey("menu") as! MainMenuScene).highestLevel = 25
                    }
                    //&&
                ...
                if theKraken!.isDead && droppedChest && (self.childNodeWithName("chest") as! TreasureChest).open{
                    dropLoot("level25", scene: self, position: CGPointMake(self.frame.midX, self.frame.midY), size: CGSizeMake(30, 30))
                    droppedItem = true
