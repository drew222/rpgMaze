...
class World1Level9: SKScene, SKPhysicsContactDelegate  {
    ...
    override func didMove(to view: SKView) {
        ...
        //shells
        //top diagnol
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX - 40, self.frame.midY + 320)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX - 120, self.frame.midY + 240)))
        if !(is5 || isPad) {
            self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX - 200, self.frame.midY + 160)))
        }
        //2nd from top diag
        if !(is5 || isPad) {
            self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX + 20, self.frame.midY + 200)))
        }
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX + 100, self.frame.midY + 280)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX - 60, self.frame.midY + 120)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX - 140, self.frame.midY + 40)))
        //mid diagnol
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX, self.frame.midY)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX + 80, self.frame.midY + 80)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX + 160, self.frame.midY + 160)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX - 80, self.frame.midY - 80)))
        self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.midX - 160, self.frame.midY - 160)))
        ...
        if (theBomber!.isDead || theHero!.life ?? <#default value#> <= 0) && !levelOver{
            ...
            if (self.childNode(withName: "gold") == nil && self.childNode(withName: "item") == nil && droppedItem) || theHero!.life ?? <#default value#> <= 0{
                ...
                if theHero!.life ?? <#default value#> <= 0 {
                    ...
                    if (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel < 9{
                        persistentData.set(9, forKey: "highestLevel")
                        (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel = 9
                    }
                    //&&
            ...
            else if (self.childNodeWithName("item") == nil && self.childNode(withName: childNodewithName,: "gold") == nil){
                if theBomber!.isDead && droppedChest && (self.childNode(withName: "chest") as! TreasureChest).open{
                    dropLoot(level: "level9", scene: self, position: CGPointMake(self.frame.midX, self.frame.midY), size: CGSizeMake(30, 30))
                    droppedItem = true
