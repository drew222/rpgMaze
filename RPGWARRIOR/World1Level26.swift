...
class World1Level26: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        if isPlus{
            //crabs
            self.addChild(MudCrab.crabAtPosition(startPos: CGPointMake(25, 140), endPosition: CGPointMake(self.frame.maxX - 25, 140)))
        ...
        //crabs
            if is5 || isPad{
                self.addChild(MudCrab.crabAtPosition(startPos: CGPointMake(25, 100), endPosition: CGPointMake(self.frame.maxX - 25, 100)))
            }else{
                self.addChild(MudCrab.crabAtPosition(startPos: CGPointMake(25, 100), endPosition: CGPointMake(self.frame.maxX - 25, 100)))
                self.addChild(MudCrab.crabAtPosition(startPos: CGPointMake(25, self.frame.maxY - 130), endPosition: CGPointMake(self.frame.maxX - 25, self.frame.maxY - 130)))
            }
        //shells
        //mid column c = x d = y
        ...
        for touch in touches{
            if !inkSplatted{
                ...
                if self.childNode(withName: "yesText")!.contains((touch ).location(in: self)){
                    let newLevel1 = World1Level26(size: self.frame.size)
    ...
    override func update(_ currentTime: CFTimeInterval) {
        ...
        if currentTime - lastFireball  > wizardAttackSpeed && !droppedChest && !inkSplatted{
            self.lastFireball = currentTime
        ...
        if (theWizard!.isDead || theHero!.life ?? <#default value#> <= 0) && !levelOver{
            ...
            if (self.childNode(withName: "gold") == nil && self.childNode(withName: "item") == nil && droppedItem) || theHero!.life ?? <#default value#> <= 0{
                ...
                if theHero!.life ?? <#default value#> <= 0 {
                    ...
                    if (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel < 26{
                        persistentData.set(26, forKey: "highestLevel")
                        (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel = 26
                    }
                    //&&
                ...
                if theWizard!.isDead && droppedChest && (self.childNode(withName: "chest") as! TreasureChest).open{
                    dropLoot(level: "level26", scene: self, position: CGPointMake(self.frame.midX, self.frame.midY), size: CGSizeMake(30, 30))
                    droppedItem = true
