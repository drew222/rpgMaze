...
class World1Level6: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        //crabs
        if isPlus{
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.midX - 55, self.frame.maxY - 200), endPosition: CGPointMake(self.frame.midX - 55, self.frame.maxY - 435)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.midX + 25, self.frame.maxY - 435), endPosition: CGPointMake(self.frame.midX + 25, self.frame.maxY - 200)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(65, self.frame.maxY - 250), endPosition: CGPointMake(65, 130)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.maxX - 50, 140), endPosition: CGPointMake(self.frame.midX - 50, 140)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.midX - 50, 220), endPosition: CGPointMake(self.frame.maxX - 50, 220)))
        } else if !(isPad || is5){
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.midX - 40, self.frame.maxY - 200), endPosition: CGPointMake(self.frame.midX - 40, self.frame.maxY - 360)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.midX + 40, self.frame.maxY - 360), endPosition: CGPointMake(self.frame.midX + 40, self.frame.maxY - 200)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(65, self.frame.maxY - 250), endPosition: CGPointMake(65, 130)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.maxX - 50, 140), endPosition: CGPointMake(self.frame.midX - 50, 140)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.midX - 50, 220), endPosition: CGPointMake(self.frame.maxX - 50, 220)))
        }else {
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(65, self.frame.maxY - 250), endPosition: CGPointMake(65, 130)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.midX - 10, 135), endPosition: CGPointMake(self.frame.maxX - 20, 135)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.maxX - 20, 225), endPosition: CGPointMake(self.frame.midX - 10, 225)))
            self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.midX - 10, 305), endPosition: CGPointMake(self.frame.maxX - 20, 305)))
            if is5 {
                self.addChild(MiniCrab.crabAtPosition(startPos: CGPointMake(self.frame.maxX - 20, 385), endPosition: CGPointMake(self.frame.midX - 10, 385)))
            }
        }
        //shells i = y, k = x
        for i in stride(from: 100, to: Int(self.frame.maxY - 100), by: 80) {
            for k in stride(from: 30.0, to: Double(self.frame.maxX), by: 80) {
                self.addChild(MineNode.mineAtPos(position: CGPointMake(CGFloat(k), CGFloat(i))))
                if Double(self.frame.maxX) - k > 40 && Double(self.frame.maxX) - k < 80 {
                    self.addChild(MineNode.mineAtPos(position: CGPointMake(CGFloat(k + 40), CGFloat(i))))
                }
            }
        }
        if isPlus && !(isPad || is5){
            self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.maxX - 105, 260)))
        }
        else if !(isPad || is5){
            self.addChild(MineNode.mineAtPos(position: CGPointMake(self.frame.maxX - 65, 260)))
        }
        ...
        if (theBomber!.isDead || theHero!.life! <= 0) && !levelOver{
            ...
            if (self.childNode(withName: "gold") == nil && self.childNode(withName: "item") == nil && droppedItem) || theHero!.life! <= 0{
                ...
                if theHero!.life! <= 0 {
                    ...
                    if (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel < 6{
                        persistentData.set(6, forKey: "highestLevel")
                        (self.userData?.object(forKey: "menu") as! MainMenuScene).highestLevel = 6
                    }
                    //&&
                ...
                if theBomber!.isDead && droppedChest && (self.childNode(withName: "chest") as! TreasureChest).open{
                    dropLoot(level: "level6", scene: self, position: CGPointMake(self.frame.midX, self.frame.midY), size: CGSizeMake(30, 30))
                    droppedItem = true
